...
using KinectProvider.Native;

namespace KinectProvider
{
    /// <summary>
    /// Use a hidden form on top for painting cursors, has problems when dominant conten is on top like Windows 8 start screen
    ...
    class DebugCursor : Form,ICursor
    {
        ...
        {
            if (currentBitmap == null || currentBitmap.Width != value.Width || currentBitmap.Height != value.Height)
            {
                feedbackImage = new Bitmap(value.Width, value.Height);
                Width = value.Width;
                Height = value.Height;
                xCenter = (Width / 2);
                yCenter = (Height / 2);
        ...
        private void Render(Graphics g)
        {
            ...
            //hide problems with wrong bitmaps
            try
            {
